...
public class  ColorCheck: MonoBehaviour
{
    ...
    public Material mat, m1, m2, m3, m4, m5;
    ...
    {
        LAB lab1 = new LAB(color1, illuminant);
        LAB lab2 = new LAB(color2, illuminant);
        return lab1.DeltaE(lab2) <= delta;
    }
    ...
    void Update()
    {
        if (AreSimilar(mat.color, m1.color, 10) || AreSimilar(mat.color, m2.color, 10) || AreSimilar(mat.color, m3.color, 10) || AreSimilar(mat.color, m4.color, 10) || AreSimilar(mat.color, m5.color, 10)
             || AreSimilar(m1.color, m3.color, 10) || AreSimilar(m1.color, m4.color, 10) || AreSimilar(m1.color, m5.color, 10) || AreSimilar(m1.color, m2.color, 10)
             || AreSimilar(m2.color, m3.color, 10) || AreSimilar(m2.color, m4.color, 10) || AreSimilar(m2.color, m5.color, 10)
             || AreSimilar(m3.color, m4.color, 10) || AreSimilar(m3.color, m5.color, 10) || AreSimilar(m3.color, m1.color, 10)
             || AreSimilar(m4.color, m5.color, 10)) 
        {error.SetActive(true);
            Debug.Log("Цвета граней одинаковые!");
            
            button1.interactable = false;
            button2.interactable = false;
        }
            
        else {button1.interactable = true;
